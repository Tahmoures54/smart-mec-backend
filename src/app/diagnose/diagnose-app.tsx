        ...
        <form onSubmit={onSubmit} className="space-y-4 rounded-2xl border border-white/10 bg-[#1A120E] p-5">
          <div className="flex gap-2">
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm ${
                mode === 'text' ? 'bg-orange-500 text-white' : 'bg-white/5'
              }`}
            >
              شرح مشکل
            </button>
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm ${
                mode === 'audio' ? 'bg-orange-500 text-white' : 'bg-white/5'
              }`}
            >
              صدای موتور
            </button>
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              {carsMeta.catalogTotal ? (
                <span className="text-[11px] text-amber-200/55">
                  {carsMeta.catalogTotal.toLocaleString('fa-IR')} مدل ·{' '}
                  {(carsMeta.brandCount ?? 0).toLocaleString('fa-IR')} برند
                </span>
              ) : null}
            </div>
            <input
              ...
              placeholder="جستجو بین صدها مدل — مثلاً پژو ۲۰۶ یا دنا"
              className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 outline-none focus:border-orange-400"
            />
            {carsMeta.categories && carsMeta.categories.length > 0 ? (
              <div className="flex gap-1.5 overflow-x-auto pb-1">
                ...
                {carsMeta.categories.slice(0, 10).map((cat) => (
                  <button
                    key={cat.id}
                    type="button"
                    onClick={() => {
                  ...
                  >
                    {cat.label}
                    <span className="mr-1 opacity-60">{cat.count.toLocaleString('fa-IR')}</span>
                  </button>
                ))}
              </div>
            ) : null}
          ...
          {carMenuOpen ? (
            <div className="max-h-72 overflow-y-auto overscroll-contain rounded-xl border border-white/10 bg-black/40 shadow-lg">
              <div className="sticky top-0 z-[1] border-b border-white/5 bg-[#1A120E]/px-3 py-2 text-[11px] text-amber-100/50">
                {carsMeta.total > 0
                  ? `${carsMeta.total.toLocaleString('fa-IR')} نتیجه — اسکرول کن، لیست ادامه دارد`
                  : 'در حال بارگذاری کاتالوگ…'}
              </div>
              {carsByBrand.map(([brand, list]) => (
                <div key={brand}>
                  <div className="sticky top-8 bg-orange-500/10 px-3 py-1.5 text-xs font-bold text-orange-200/90">
                    {brand}
                    <span className="mr-2 font-normal text-amber-100/40">
                      {list.length.toLocaleString('fa-IR')} مدل در این صفحه
                    </span>
                  </div>
                  {list.map((car) => (
                    <button
                    ...
                    >
                      ...
                      {car.engine ? (
                        <span className="text-amber-100/40"> · {car.engine}</span>
                      ) : null}
                      {car.isPopular ? (
                        <span className="mr-2 rounded bg-amber-400/15 px-1.5 py-0.5 text-[10px] text-amber-200">
                          پرتیراژ
                        </span>
                      ) : null}
                    </button>
              ...
              {carsMeta.hasMore ? (
                <button
                  ...
                  onClick={() => void loadCars(query, { offset: carsOffset, append: true })}
                  className="w-full border-t border-white/10 px-3 py-3 text-sm text-orange-300 hover:bg-white/5 disabled:opacity-50"
                >
                  {carsLoading ? 'در حال بارگذاری…' : 'نمایش مدل‌های بیشتر ↓'}
                </button>
              ) : cars.length > 0 ? (
                <p className="border-t border-white/10 px-3 py-2 text-center text-[11px] text-amber-100/40">
                  پایان نتایج این فیلتر — {carsMeta.total.toLocaleString('fa-IR')} مدل
                </p>
              ) : null}
              <button
                type="button"
                ...
                className="block w-full border-t border-white/10 px-3 py-2 text-right text-sm text-orange-300 hover:bg-white/5"
              >
