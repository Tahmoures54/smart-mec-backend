...
export function DiagnoseApp() {
  ...
  async function sendOtp() {
    ...
    setLoading(true);
    try {
      const { ok, body } = await api<{ otp?: string; error?: string }>('/api/account', {
        method: 'POST',
    ...
    <div className="mx-auto max-w-6xl px-4 py-10">
      ...
      <form onSubmit={onSubmit} className="space-y-4 rounded-2xl border border-white/10 bg-[#1A120E] p-5">
        ...
        {mode === 'text' ? (
          <div>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4} placeholder="مثلاً وقتی سرد است تق‌تق از موتور می‌آید…" className="mt-1 w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2" />
          </div>
        ) : (
          <div className="space-y-2">
            <div className="flex flex-wrap gap-2">
              {!recording ? (
                ...
                <button type="button" onClick={stopRecording} className="rounded-xl bg-red-500/80 px-4 py-2 text-sm">توقف ضبط</button>
              )}
      ...
      <style
        dangerouslySetInnerHTML={{
          __html: `
        @keyframes diagnose-result-arrive {
          0% { transform: translateY(16px) scale(0.98); opacity: 0.5; }
          35% { transform: translateX(-6px) translateY(0) scale(1); opacity: 1; }
          55% { transform: translateX(5px); }
          75% { transform: translateX(-3px); }
          100% { transform: translateX(0); opacity: 1; }
        }
        .diagnose-result-arrive {
          animation: diagnose-result-arrive 0.55s cubic-bezier(0.22, 1, 0.36, 1);
        }
      `,
        }}
      />
      ...
      {history.length > 0 ? (
        <div className="mt-10">
          <h2 className="text-lg font-bold text-amber-100">تاریخچه اخیر</h2>
          <ul className="mt-3 space-y-2">
            {history.map((item) => (
              <li key={item.id}>
                <button type="button" onClick={() => openHistory(item)} className="w-full rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-right text-sm hover:bg-white/5">
                  <span className="text-amber-100/80">{item.description.slice(0, 80)}</span>
                </button>
              </li>
            ))}
          </ul>
